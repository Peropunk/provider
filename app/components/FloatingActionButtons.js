...
const FloatingActionButtons = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      // Get the SearchBarHero section element
      const searchBarHero = document.querySelector('section.bg-gradient-to-br');
      
      if (searchBarHero) {
        const searchBarHeroBottom = searchBarHero.offsetTop + searchBarHero.offsetHeight;
        const scrollPosition = window.scrollY + window.innerHeight;
        
        // Show back to top button only when scrolled past the SearchBarHero section
        setShowBackToTop(scrollPosition > searchBarHeroBottom);
      }
    };
    ...
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-4">
      ...
      {showBackToTop && (
        <button
          ...
          className="group flex items-center justify-center w-14 h-14 bg-white shadow-lg rounded-full border border-gray-200 hover:bg-slate-100 transition relative animate-in slide-in-from-bottom-2 duration-300"
